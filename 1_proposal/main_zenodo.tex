...
\date{
    ...
    October 2023 \textit{(available online July 2025)}
}
...
\begin{tikzpicture}[remember picture, overlay]
    \node[xshift=-10.3cm, yshift=+26cm, font=\small] at (current page.south east)
    {
    \href{https://doi.org/10.5281/zenodo.14575784}{\texttt{doi:10.5281/zenodo.14575784}}
    \hspace{3cm} \href{https://creativecommons.org/licenses/by/4.0/deed.en}{Under a Creative Commons Attribution 4.0 License}
    };
\end{tikzpicture}
...
\hline
Michael P. Weinold & \censor{\texttt{michael@weinold.ch}} & \censor{\texttt{+41 75 500 70 70}} \\
Philippe Schultheiss & \censor{\texttt{phschultheiss@protonmail.ch}} & \censor{\texttt{+41 79 768 53 15}} \\
Mark C. Ballandies & \censor{\texttt{bcmark@protonmail.com}} & \censor{\texttt{+41 78 899 87 85}}
\end{NiceTabularX}
...
3 & 25.10.2023 & \texttt{82181de} & version submitted to the Foundation \\
4 & 20.07.2025 & \texttt{b807306} & version for public release without copyrighted images
\end{NiceTabularX}
...
\begin{minipage}{0.325\textwidth}
    \begin{figure}[H]
        \centering
        \begin{tikzpicture}
            \draw[line width=1pt, black] (0,0) rectangle (50mm, 188.06mm); % Width 50mm, height based on aspect ratio
            \fill[white] (0,0) rectangle (50mm, 188.06mm); % White background
            \node at (25mm, 94.03mm) [text width=45mm, align=center] {Figure not shown for copyright reasons. \\ See \textit{Evolution Chart of Design - 1930} on page 76 in "Industrial Design" by Raymond Loewy (1979), \texttt{ISBN 9789029529150}.}; % Centered text with padding
        \end{tikzpicture}
        \caption{
...
The second reason lies in what has been perceived as "diminishing carbon emissions returns" associated with extending building service-life.
...
In fact, a self-understanding that was shared among the founding fathers of modern architecture, from Mies van der Rohe to Le Corbusier was that of the architect as the \textit{seer and sociological priest} \cite[Sec. 4]{hughes_shock_1993} that, through architecture, was to re-educate the public such that they might come to appreciate their brutalist or modernist designs. The modern version of this is perhaps best exemplified by the reaction of Corbu's Swiss compatriot Rudolf Guyer, who in 1955 designed a high-rise residential apartment in southern Zurich. When the building was voted "Switzerland's Ugliest Building" in 2018 by readers of the daily newspaper \textit{20 Minuten}, he declared:
...
% Contemporary architects came out to defend Guyer's work as well, stating that

% \textit{"Grundsätzlich haben sie [Architekten] nicht die Pflicht, ihre Werke Laien schmackhaft zu machen (solange sie nicht als Bauherrschaft am Drücker sitzen). Problematisch wird es aber, wenn Fachverbände (...) an Fördertöpfe gelangen wollen. Diese werden auch durch die Steuergelder ignoranter Laien gefüllt."} (en.: In principle, they [the architects] have no obligation to make their works palatable to laypeople (as long as they are not in charge as the client). However, it becomes problematic when professional associations (...) want to acquire funding. This funding is also provided by the tax money of ignorant laypeople.) - Manuel Pestalozzi \cite{pestalozzi_hasslichkeit_2018}
...
"Democracy by design" \cite{helbing_ethics_2021}\cite{helbing_democracy_2023} combines these methods from value-sensitive design and applies them to democracy by keep the human-in-the loop of important decision making in society. The key elements of this approach is the acknowledgement that every citizen should have the right to express their opinion on a system that affect them and be able to improve them \cite{helbing_ethics_2021}.
\end{multicols}
\vspace{-5mm}

\begin{figure}[ht!]
    \centering
    \subfloat{{\includegraphics[height=4cm]{figures/vienna_1.jpg} }}
    \subfloat{{
        \begin{tikzpicture}
            \draw[line width=1pt, black] (0,0) rectangle (7.48cm, 4cm); % Width based on aspect ratio (5143/2751 ≈ 1.8699) at height=4cm
            \fill[white] (0,0) rectangle (7.48cm, 4cm); % White background
            \node at (3.74cm, 2cm) [text width=6.98cm, align=center] {Image not shown for copyright reasons. See \href{https://www.wienschauen.at/schoenbrunner-strasse-211\%E2\%80\%8B-abriss-neubau/}{\textit{Schönbrunner Strasse 211: Neubau nach Abriss}} by Georg Scherer.}; % Centered text with padding, escaped URL
        \end{tikzpicture}
    }}
...
\begin{figure}[ht!]
    ...
    \subfloat{{
        \begin{tikzpicture}
            \draw[line width=1pt, black] (0,0) rectangle (7.48cm, 4cm); % Width based on aspect ratio (5143/2751 ≈ 1.8699) at height=4cm
            \fill[white] (0,0) rectangle (7.48cm, 4cm); % White background
            \node at (3.74cm, 2cm) [text width=6.98cm, align=center] {Image not shown for copyright reasons. See \href{https://index.hu/urbanista/2015/12/16/eredmenyt_hirdettek_a_kossuth_teri_mtesz-szekhaz/}{\textit{Ilyen most. Milyen lesz az átalakítás után?}} by Zubreczki Dávid.}; % Centered text with padding
        \end{tikzpicture}
    }}
    \subfloat{{
        \begin{tikzpicture}
            \draw[line width=1pt, black] (0,0) rectangle (7.48cm, 4cm); % Width based on aspect ratio (5143/2751 ≈ 1.8699) at height=4cm
            \fill[white] (0,0) rectangle (7.48cm, 4cm); % White background
            \node at (3.74cm, 2cm) [text width=6.98cm, align=center] {Image not shown for copyright reasons. See \href{https://pestbuda.hu/cikk/20190829_szepul_a_hultl_fele_haz_a_parlament_leendo_irodahaza_a_kossuth_teren}{\textit{A Kossuth téren felépített új parlamenti irodaház az 1928-ban tervezett homlokzattal}} by Both Balázs.}; % Centered text with padding
        \end{tikzpicture}
    }}
    \caption{Before-and-after: Demolition and reconstruction of a 1970s office building on Kossuth Lajos Square in Budapest, adjacent to the Hungarian Parliament Building, as documented by Michael Diamant of the architecture watchblog \texttt{newtrad.org}. The brutalist style design was intended to replace the unrealized wing of an earlier neo-classical style building. This wing was finally constructed according to the original design following the demolition of the 1970s building. Image sources: Adapted from original photographs used by Michael Diamant in \cite{natalia_michael_2021}}
    \label{fig:budaptest}
\end{figure}
...
However, biophilic design can transcend the aesthetic dimension. While nature-inspired functional aspects of construction are often described under the term bio-mimicry, the difference is only semantic. \cref{fig:ventilation} shows what is perhaps the most famous example of functional bio-mimicry in the built environment: The design of building-scale ventilation systems based on the thermo-regulatory designs of termite mounds.

We believe that biophilic design in its aesthetic dimension is an answer to the problems of denizen wellbeing highlighted in \cref{sec:ugliness}, while biophilic design in its functional dimension can help solve the challenges of sustainability in construction discussed in \cref{sec:construction}. 

It would seem that the academic community is in agreement with our notions: The academic interest in biophilic design has been growing exponentially since 2012, some of which can likely be attributed to the term itself being "in fashion" \footnote{Compare the results of the Elsevier Scopus publication index for the search: \texttt{TITLE-ABS-KEY(biophilic AND design)}}. But will biophilic design emerge as a major new force, a new \textit{Strömung} (en.: architectural tendency), healing wounds that exposed concrete and steel have torn in the fabric of European cities? Will the use of natural materials be able to reduce the environmental burdens of the construction sector, or will architects simply adorn their designs with the odd tree here-and-there to make sure their designs don't look completely out of time? Will ornaments, inspired by forms of nature, once again bloom on the facades - or will real living plants grow on walls, balconies and terraces? Will new developments in neuroscience and psychology be able to help us design better, more beautiful buildings?
...
\begin{figure}[ht!]
    ...
    \subfloat{{
        \begin{tikzpicture}
            \draw[line width=1pt, black] (0,0) rectangle (7.792cm, 5cm); % Width based on aspect ratio (427/274 ≈ 1.5584) at height=4cm
            \fill[white] (0,0) rectangle (7.792cm, 5cm); % White background
            \node at (3.896cm, 2.5cm) [text width=7.292cm, align=center] {Image not shown for copyright reasons. See plan of Portcullis House in \textit{Hopkins2: the work of Michael Hopkins and Partners} by Colin Davies et al. (2001), \texttt{ISBN 978071483925}.}; % Centered text with padding
        \end{tikzpicture}
    }}
    \subfloat{{
        \begin{tikzpicture}
            \draw[line width=1pt, black] (0,0) rectangle (7.792cm, 5cm); % Width based on aspect ratio (427/274 ≈ 1.5584) at height=5cm
            \fill[white] (0,0) rectangle (7.792cm, 5cm); % White background
            \node at (3.896cm, 2.5cm) [text width=7.292cm, align=center] {Image not shown for copyright reasons. See Figure 2 in \href{https://www.researchgate.net/publication/255650482}{\textit{Beyond biomimicry: What termites can tell us about realizing the living building}} by Scott Turner (2008).}; % Centered text with padding
        \end{tikzpicture}
    }}
