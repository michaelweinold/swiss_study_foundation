...
\begin{tikzpicture}[remember picture, overlay]
    \node[xshift=-10.3cm, yshift=+27cm, font=\small] at (current page.south east)
    {
...
\begin{mdframed}[linewidth=1pt, roundcorner=5pt, innerleftmargin=10pt, innerrightmargin=10pt, innertopmargin=10pt, innerbottommargin=10pt, linecolor=black, backgroundcolor=white, userdefinedwidth=\textwidth]
    ...
    \textbf{In this section, we present one of the most well-known polemics against traditional architecture generally and against ornament more specifically. Here, we include the full text, both because of its cultural significance and because of its non-negligible comedic value.}
...
\begin{mdframed}[linewidth=1pt, roundcorner=5pt, innerleftmargin=10pt, innerrightmargin=10pt, innertopmargin=10pt, innerbottommargin=10pt, linecolor=black, backgroundcolor=white, userdefinedwidth=\textwidth]
    Measure beauty? Quantify the aesthetic experience? What a preposterous idea! But for philosophers from Plato \cite{plato_symposium_1795} to Hume \cite{hume_iv_1757}, beauty was far from subjective - thereby a beautiful building would be one that exhibits order, symmetry, proportion, harmony, and purpose, and that reveals the essence and goodness of the \textit{Form of Beauty}. Neuroscience and psychology have only recently been able to contribute significantly to this debate. Unperturbed by the intellectual debate on the exact ontological nature of beauty, large-scale scientific studies have advanced our understanding of the aesthetic features which endow the built environment with healing qualities. Initial results point to natural forms, which for millennia have inspired the design of man-made structures, and the many geometric forms that take inspiration from them. Absent in modernist designs, they permeate the facades of many historical buildings in Europe and beyond. Lavdas and Salingaros have gone furthest, proposing a methodology to determine what they boldly claim to be an \textit{"objective measure for beauty"}, partly based on eye-tracking, hinted at in \cref{fig:heatmap}. \\
    \textbf{In this section, we present excerpts adapted from a 2022 article by Lavdas and Salingaros \cite{lavdas_architectural_2022}\footnote{Published under a \href{https://creativecommons.org/licenses/by/4.0/}{Creative Commons 4.0 License}}. It outlines the context in which the authors have proposed their novel method for quantifying beauty, but omits technical details on the computation of the scale, which will be discussed at length during the Academy. Note the contrast between this work and \cref{sec:ornamentcrime}.}
\end{mdframed}
...
\begin{figure}[h]
  ...
  \caption{Left: Two designs for a building facade. Right: A \textit{Visual Attention Span (VAS)} heatmap, computed to approximate the degree to which different facade designs capture viewer attention. As intuitively expected, the feature-rich pillar capital and lower-side cornice of the right facade retains visual attention to a much higher degree.}
  \label{fig:heatmap}
\end{figure}
...
\begin{mdframed}[linewidth=1pt, roundcorner=5pt, innerleftmargin=10pt, innerrightmargin=10pt, innertopmargin=10pt, innerbottommargin=10pt, linecolor=black, backgroundcolor=white, userdefinedwidth=\textwidth]
    ...
    \textbf{In this section, we present two videos documenting the origins of the Architecture Uprising. We also present an excerpt from a recent Bloomberg article that investigated the impact of this grassroots movement.}
\end{mdframed}
...
\begin{minipage}[t]{0.49\linewidth}
    \centering
    \begin{tikzpicture}
        \draw[line width=1pt, black] (0,0) rectangle (\linewidth, 0.38\linewidth); % Height increased from 0.27\linewidth to 0.30\linewidth
        \fill[white] (0,0) rectangle (\linewidth, 0.38\linewidth); % White background
        \node at (0.5\linewidth, 0.19\linewidth) [text width=\linewidth-0.5cm, align=center] {Thumbnail not shown for copyright reasons.}; % Centered text with updated y-coordinate
    \end{tikzpicture}
...
\begin{mdframed}[linewidth=1pt, roundcorner=5pt, innerleftmargin=10pt, innerrightmargin=10pt, innertopmargin=10pt, innerbottommargin=10pt, linecolor=black, backgroundcolor=white, userdefinedwidth=\textwidth]
    ...
    \textbf{In this section, we present a figure adapted from a recent publication by Röck et al.\footnote{Published under a \href{https://creativecommons.org/licenses/by/4.0/}{Creative Commons 4.0 License}} \cite{rock_embodied_2020} that shows how lifetime carbon emissions can differ for older and new construction. Since this work focuses only on the \textit{function} of buildings, we also present a recent scientific comment advocating for a focus on architectural \textit{beauty} to improve sustainability \cite{piotrowski_focusing_2024}.}
\end{mdframed}
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/emissions.pdf}
...
\vfill
\begin{center}
    Excerpt from scientific publication not shown for copyright reasons. \\
    See \textit{Focusing on architectural beauty
to reduce construction waste} by Piotr Piotrowski (2024), Nature Reviews Earth \& Environment, \href{https://doi.org/10.1038/s43017-024-00609-y}{\texttt{doi:10.1038/s43017-024-00609-y}}.
\end{center}
\vfill
\clearpage
